#!/usr/bin/env python3
"""
Slot receiver outcome visualization — v2.

Updated filter: top-quartile career slot rate (>=64.3%, snap-weighted), R1-2 or college E[tier] > 1.0, >200 FBS routes.

Outputs: wr_data/charts/slot_receiver_outcomes_v2.png
"""
...
def main():
    ...
    master["career_routes"] = master["name"].map(player_routes)
    ...
    # Top quartile slot rate threshold (75th percentile of all 200+ route WRs = 64.3%)
    TOP_QUARTILE = 64.3
    ...
    slot = candidates[
        (candidates["career_routes"] >= 200) & (candidates["career_slot_rate"] >= TOP_QUARTILE)
    ].copy()
    ...
    print(f"Players in chart: {len(slot)}")
    for _, r in slot.iterrows():
        print(f"  {r['name']:<25s} slot={r['career_slot_rate']:5.1f}%  tier={r['display_tier']}")

    fig, ax = plt.subplots(figsize=(12, 9))
    fig.suptitle(f"Top-Quartile Slot WRs (>={TOP_QUARTILE}% career slot)  (R1-2 or E[tier] > 1.0, >200 routes)",
    ...
    out_path = os.path.join(OUT_DIR, "slot_receiver_outcomes_v2.png")
