#!/usr/bin/env python3
"""
v11 WR Holdout Evaluation: Log DC + 60/40 Ensemble.

Changes from v9:
  - v10: Draft capital: log-scaled (was sqrt)
  - v11: Ensemble: 60% Bayesian / 40% XGBoost (was 75/25)

Trains on 2017-2021, predicts 2022-2024 (same protocol).
Produces ensemble predictions with actual tiers for comparison.

Outputs:
  - wr_data/outputs/holdout_predictions_v11.csv
"""
...
print(f"Draft capital: log-scaled (10 - (10/ln(261)) * ln(pick+1))")
...
# === Evaluate ===
print("\n" + "=" * 70)
print("v11 HOLDOUT EVALUATION (2022-2024)")
print("Log DC + 60/40 Bayesian/XGBoost")
print("=" * 70)
...
# === v9 comparison ===
print("\n" + "=" * 70)
print("v9 vs v10 COMPARISON (Ensemble Full)")
print("=" * 70)
v9_metrics = {
    "LogLoss": 0.771,
    "Brier": 0.343,
...
v10_ll = results["Ensemble Full"]["ll"]
v10_brier = results["Ensemble Full"]["brier"]

# Compute v10 AUCs
v10_aucs = {}
for threshold, tlabel in zip(THRESHOLDS, THRESHOLD_LABELS):
    y_bin = (actual >= threshold).astype(int)
    pred = full_probs[:, threshold:].sum(axis=1)
    v10_aucs[tlabel] = roc_auc_score(y_bin, pred) if 0 < y_bin.sum() < len(y_bin) else float("nan")

print(f"\n  {'Metric':<20s} {'v9 (sqrt 75/25)':>15s} {'v10 (log 60/40)':>15s} {'Delta':>10s}")
print(f"  {'-'*20} {'-'*15} {'-'*15} {'-'*10}")
print(f"  {'LogLoss':<20s} {0.771:>15.4f} {v10_ll:>15.4f} {v10_ll - 0.771:>+10.4f}")
print(f"  {'Brier':<20s} {0.343:>15.4f} {v10_brier:>15.4f} {v10_brier - 0.343:>+10.4f}")
for tlabel in THRESHOLD_LABELS:
    v9_key = f"{tlabel} AUC"
    v9_val = v9_metrics.get(v9_key, None)
    v10_val = v10_aucs[tlabel]
    if v9_val is not None:
        print(f"  {v9_key:<20s} {v9_val:>15.3f} {v10_val:>15.3f} {v10_val - v9_val:>+10.3f}")
    else:
        print(f"  {tlabel + ' AUC':<20s} {'--':>15s} {v10_val:>15.3f}")
...
# Save
out_path = os.path.join(DATA_DIR, "outputs", "holdout_predictions_v11.csv")
